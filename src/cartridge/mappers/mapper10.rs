// Mapper 10 (MMC4) - Advanced mapper with latch-based CHR banking (similar to MMC2)
//
// Memory Layout:
...
// - PRG-ROM size: up to 256KB
// - CHR-ROM size: up to 128KB
//
// Register Interface:
// - $A000-$AFFF: PRG-ROM bank select (4 bits)
...
//
// Main difference from MMC2 (Mapper 9):
// - MMC4 uses 16KB PRG banking instead of 8KB
// - Different PRG bank size affects the fixed bank location
...
/// Mapper 10 implementation (MMC4)
///
/// MMC4 was used by Fire Emblem (Japan) and uses a similar latch-based
/// CHR banking system to MMC2, but with 16KB PRG banks instead of 8KB.
pub struct Mapper10 {
    /// PRG-ROM data
    prg_rom: Vec<u8>,
    /// CHR-ROM data
    ...
    // Internal registers
    /// PRG-ROM bank select (for $8000-$BFFF)
    prg_bank: u8,
...
impl Mapper10 {
    /// Create a new Mapper10 instance from a cartridge
    ///
    /// # Arguments
    ...
    pub fn new(cartridge: Cartridge) -> Self {
        ...
        // Validate that we have at least one bank of each type
        assert!(
            prg_banks > 0 && chr_banks > 0,
            "Mapper10 requires at least one PRG and CHR bank (got {} PRG, {} CHR)",
            prg_banks,
            chr_banks
        );
...
impl Mapper for Mapper10 {
    fn cpu_read(&self, address: u16) -> u8 {
        match address {
            ...
            0xC000..=0xFFFF => {
                ...
                let base = (self.prg_banks - 1) * PRG_BANK_SIZE;
    ...
    fn ppu_read(&self, address: u16) -> u8 {
        // Update latches based on address (this is the critical behavior for MMC4)
    ...
    fn ppu_write(&mut self, address: u16, _value: u8) {
        ...
        self.update_latch(address);
        // MMC4 uses CHR-ROM, writes are ignored
    }
...
mod tests {
    ...
    #[test]
    fn test_mapper10_creation() {
        let cartridge = create_test_cartridge(8, 32);
        let mapper = Mapper10::new(cartridge);
    ...
    fn test_prg_bank_switching() {
        let mut cartridge = create_test_cartridge(8, 32);
        ...
        let mut mapper = Mapper10::new(cartridge);
        ...
        // Last bank should be fixed (bank 7)
        assert_eq!(mapper.cpu_read(0xC000), 7);
    }

    #[test]
    fn test_16kb_prg_banking() {
        ...
        // Test that switchable bank covers full 16KB
        assert_eq!(mapper.cpu_read(0x8000), 0);
        assert_eq!(mapper.cpu_read(0xBFFF), 0);

        // Test that fixed bank covers full 16KB and is last bank
        assert_eq!(mapper.cpu_read(0xC000), 15);
    ...
    #[test]
    fn test_difference_from_mmc2() {
        // Main difference: MMC4 uses 16KB PRG banks vs MMC2's 8KB
        let mut cartridge = create_test_cartridge(8, 32);
        ...
        // Entire 16KB should be from bank 2
        assert_eq!(mapper.cpu_read(0x8000), 2);
        assert_eq!(mapper.cpu_read(0x9000), 2);
        assert_eq!(mapper.cpu_read(0xA000), 2);
        assert_eq!(mapper.cpu_read(0xB000), 2);

        // Fixed bank is last 16KB
        assert_eq!(mapper.cpu_read(0xC000), 7);
        assert_eq!(mapper.cpu_read(0xD000), 7);
        assert_eq!(mapper.cpu_read(0xE000), 7);
        assert_eq!(mapper.cpu_read(0xF000), 7);
    }
}
